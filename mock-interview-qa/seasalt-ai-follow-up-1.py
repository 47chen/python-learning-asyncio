...
dq = asyncio.Queue()
kitchen_throttle = asyncio.Semaphore(1) # only allow 1 consumer(chef)
...
async def chef(chef_id: int):
    ...
    while dq:
        ...
        async with kitchen_throttle:
            print(f'Chef{chef_id}: Accessing the kitchen')
            print(f'Chef{chef_id}: Starting processing Order{order_number}')
            await asyncio.sleep(1.5) # simulate IO ops
            print(f'Chef{chef_id}: Finish Order{order_number}')
            print(f'Chef{chef_id}: Releasing the kitchen')
